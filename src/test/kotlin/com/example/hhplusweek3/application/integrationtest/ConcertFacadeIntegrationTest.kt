...
class ConcertFacadeIntegrationTest(
    ...
    fun `when find available seats and all concert seats are available, then return all 50 seats`() {
        ...
        // then
        assertThat(actual.availableSchedules.first().seats.size).isEqualTo(50)
        assertThat(actual.allSchedules.first().seats.size).isEqualTo(50)
    }
    ...
    fun `when find available seats and partial seats are available, then return partial seats`() {
        ...
        val date = LocalDate.now().plusDays(2).atStartOfDay().toInstant(ZoneOffset.UTC)
        testService.resetConcertSeats()
        testService.resetAndReserveHalfSeatsInDate(date)
        val query = FindAvailableConcertSeatsQuery(date)
        ...
        // then
        assertThat(actual.availableSchedules.first().seats.size).isEqualTo(25)
        assertThat(actual.allSchedules.first().seats.size).isEqualTo(50)
    }
    ...
    fun `when finding available seats and no seats are available, then empty list`() {
        ...
        val date = LocalDate.now().plusDays(2).atStartOfDay().toInstant(ZoneOffset.UTC)
        testService.resetConcertSeats()
        testService.resetAndReserveAllSeatsInDate(date)
        val query = FindAvailableConcertSeatsQuery(date)
    ...
    fun `when find available dates, past date is not included`() {
        // given
        testService.resetConcertSeats(-5, -1)
